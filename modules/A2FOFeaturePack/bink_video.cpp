...
namespace a2fo {
    namespace {
        ...
        // Armada's shell state is shared with the native single-player flow. State 1
        // is the main menu; values above it represent nested native shell screens such
        // as Single Player. Those screens already position their Bink elements as part
        // of the UI layout, so forcing them to the live viewport corrupts the menu.
        constexpr std::uintptr_t kArmadaShellStateRva = 0x003a8980;
        constexpr std::int32_t kArmadaMainMenuShellState = 1;
        ...
        volatile LONG g_shell_bypass_log_count = 0;
        volatile LONG g_last_shell_state = 0x7fffffffL;
        ...
        bool preserve_native_shell_bink(std::int32_t* shell_state_out = nullptr) noexcept {
            if (!g_armada) return false;

            const auto* shell_state = reinterpret_cast<const std::int32_t*>(
                at(g_armada, kArmadaShellStateRva));
            const std::int32_t state = *shell_state;
            if (shell_state_out) *shell_state_out = state;

            const LONG previous = InterlockedExchange(
                &g_last_shell_state, static_cast<LONG>(state));
            if (previous != static_cast<LONG>(state)) {
                char message[160]{};
                std::snprintf(
                    message, sizeof(message),
                              "Armada Bink shell state changed to %ld; scaling=%s",
                              static_cast<long>(state),
                              state > kArmadaMainMenuShellState ? "bypassed" : "enabled");
                log_message(message);
            }

            // Main menu (1) and pre-shell/startup states retain FeaturePack scaling.
            // Nested native shell screens already supply their own transformed Bink
            // rectangles and must be left untouched.
            return state > kArmadaMainMenuShellState;
        }

        void log_native_shell_bypass_once(std::int32_t shell_state) noexcept {
            if (InterlockedIncrement(&g_shell_bypass_log_count) != 1) return;

            char message[192]{};
            std::snprintf(
                message, sizeof(message),
                          "Armada Bink scaling bypassed for native nested shell state %ld "
                          "(Single Player/menu layout preserved)",
                          static_cast<long>(shell_state));
            log_message(message);
        }
        ...
        int WINAPI armada_bink_set_dibits_scaled(
            ...
            const BITMAPINFO* bitmap_info, UINT color_use) noexcept {
                if (!device_context || !pixels || !bitmap_info || width == 0 ||
                    ...
                    std::int32_t shell_state = 0;
                if (preserve_native_shell_bink(&shell_state)) {
                    log_native_shell_bypass_once(shell_state);
                    return SetDIBitsToDevice(
                        device_context, destination_x, destination_y, width, height,
                        source_x, source_y, first_scan, scan_count, pixels, bitmap_info,
                        color_use);
            ...
            void __cdecl armada_bink_texture_render_scaled(
                const ArmadaFloatRect* requested_rectangle) noexcept {
                    ...
                    if (preserve_native_shell_bink(&shell_state)) {
                        log_native_shell_bypass_once(shell_state);
                        g_armada_bink_texture_render(requested_rectangle);
                        return;
                    }
                                                                                                           ...
                                                                                                           if (!api->patch_call(call_site,
                                                                                                               ...
                                                                                                               return false;
                                                                                                                                                                                   ...
                                                                                                                                                                                   api->log(
                                                                                                                                                                                       ...
                                                                                                                                                                                       "native nested shell screens bypass scaling");
                                                                                                                                                                                   return true;
                                       }
